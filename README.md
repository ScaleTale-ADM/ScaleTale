# ScaleTale

**ScaleTale** is a free Python/Pygame space action game created as a personal indie project.

**Current public beta:** `V71 - Music & SFX Beta`

ScaleTale combines arcade space combat, boss fights, planet selection, Scale-0 mystery events, cockpit scanning, dynamic music, sound effects and boss loot rewards.

---

## English

### Features

- Arcade space combat made with Python and Pygame.
- Multiple enemy types and cinematic boss fights.
- Planet selector with different visual atmospheres.
- Scale-0 mystery content and top-down exploration.
- Cockpit scan system before major boss fights.
- Dynamic background music by game context.
- Sound effects for weapons, bosses, cockpit, loot, impacts and pickups.
- Boss loot system with temporary upgrades and rewards.
- Spanish and English language support.

### How to Play

1. Download the latest release from GitHub.
2. Install Python 3.
3. Install Pygame:

```bash
pip install pygame
```

4. Run the main Python file:

```bash
python SCATEL_V71_MUSIC_ASSET_STRUCTURE_BETA.py
```

### Recommended Download

For normal players, use the latest `.zip` from the GitHub Releases section. It should include:

- the main `.py` file
- the `assets` folder
- documentation

### Community

Join the Scale Tale Community Discord to follow updates, report bugs, share feedback, test beta versions and discuss modded versions.

### Project Status

ScaleTale is currently in beta. The game is playable, but systems, balance, visuals and code structure are still evolving.

---

## Espanol

### Caracteristicas

- Combate arcade espacial hecho con Python y Pygame.
- Multiples tipos de enemigos y bosses cinematograficos.
- Selector de planetas con atmosferas visuales diferentes.
- Contenido misterioso de Scale-0 y exploracion top-down.
- Sistema de escaneo en cabina antes de bosses importantes.
- Musica dinamica segun el contexto del juego.
- Efectos de sonido para armas, bosses, cabina, loot, impactos y pickups.
- Sistema de botin tras derrotar bosses.
- Soporte para idioma espanol e ingles.

### Como Jugar

1. Descarga la ultima version desde GitHub Releases.
2. Instala Python 3.
3. Instala Pygame:

```bash
pip install pygame
```

4. Ejecuta el archivo principal:

```bash
python SCATEL_V71_MUSIC_ASSET_STRUCTURE_BETA.py
```

### Descarga Recomendada

Para jugadores normales, usa el `.zip` mas reciente de la seccion GitHub Releases. Deberia incluir:

- el archivo `.py` principal
- la carpeta `assets`
- documentacion

### Comunidad

Unete al Discord de Scale Tale Community para recibir novedades, reportar bugs, compartir feedback, probar betas y hablar sobre versiones modificadas.

### Estado Del Proyecto

ScaleTale esta actualmente en beta. El juego es jugable, pero los sistemas, balance, visuales y estructura del codigo siguen evolucionando.

