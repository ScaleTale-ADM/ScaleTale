...
# =====================
# El juego se dibuja siempre a 800x600 dentro de "pantalla".
# Luego se escala proporcionalmente a la ventana real sin deformar.
fullscreen = False
ventana = pygame.display.set_mode((ANCHO, ALTO), pygame.RESIZABLE)
pygame.display.set_caption("ScaleTale - V71 Music & Asset Structure Beta")
...
PLAYER_SPEED_NORMAL = 5.6
PLAYER_SPEED_SLOWED = 3.4
...
def crear_frame_online():
    # LAN Online V2:
    # El host env�a la pantalla interna 800x600 ya dibujada.
    # El cliente la muestra directamente, por lo que la imagen es mucho m�s fiel.
    try:
...
def enviar_inputs_cliente(teclas):
    if net_role == "client" and net_connected and net_client is not None:
        ...
        # WASD, flecha arriba y habilidades est�ndar.
...
def rect_jugador_principal():
    tam = 42 if estado.get("coop",False) else 50
    return pygame.Rect(estado["nave_x"], estado["nave_y"], tam, tam)

def rect_jugador_2():
    if not estado.get("coop",False):
        return None
    return pygame.Rect(estado["nave2_x"], estado["nave2_y"], 42, 42)
...
def aplicar_dano_jugador(cantidad=1):
    # Sistema de HP real.
    # Antes 1 punto de da�o equival�a aproximadamente a 1 coraz�n.
...
def cargar_imagen(nombre, tamano, color_fallback=(120,120,120)):
    try:
        ...
        return crear_sprite_procedural(nombre, tamano, color_fallback)
...
start_btn = pygame.Rect(MENU_ROW1_X + 0*(MENU_BTN_W+MENU_GAP), MENU_ROW1_Y, MENU_BTN_W, MENU_BTN_H)
options_btn = pygame.Rect(MENU_ROW1_X + 1*(MENU_BTN_W+MENU_GAP), MENU_ROW1_Y, MENU_BTN_W, MENU_BTN_H)
shop_btn = pygame.Rect(MENU_ROW1_X + 2*(MENU_BTN_W+MENU_GAP), MENU_ROW1_Y, MENU_BTN_W, MENU_BTN_H)
controls_btn = pygame.Rect(MENU_ROW1_X + 3*(MENU_BTN_W+MENU_GAP), MENU_ROW1_Y, MENU_BTN_W, MENU_BTN_H)
language_btn = pygame.Rect(MENU_ROW1_X + 4*(MENU_BTN_W+MENU_GAP), MENU_ROW1_Y, MENU_BTN_W, MENU_BTN_H)
info_btn = pygame.Rect(MENU_ROW1_X + 5*(MENU_BTN_W+MENU_GAP), MENU_ROW1_Y, MENU_BTN_W, MENU_BTN_H)
profile_btn = pygame.Rect(MENU_ROW1_X + 6*(MENU_BTN_W+MENU_GAP), MENU_ROW1_Y, MENU_BTN_W, MENU_BTN_H)

difficulty_btn = pygame.Rect(MENU_ROW2_X + 0*(MENU_BTN_W+MENU_GAP), MENU_ROW2_Y, MENU_BTN_W, MENU_BTN_H)
missions_btn = pygame.Rect(MENU_ROW2_X + 1*(MENU_BTN_W+MENU_GAP), MENU_ROW2_Y, MENU_BTN_W, MENU_BTN_H)
codex_btn = pygame.Rect(MENU_ROW2_X + 2*(MENU_BTN_W+MENU_GAP), MENU_ROW2_Y, MENU_BTN_W, MENU_BTN_H)
achievements_btn = pygame.Rect(MENU_ROW2_X + 3*(MENU_BTN_W+MENU_GAP), MENU_ROW2_Y, MENU_BTN_W, MENU_BTN_H)
upgrades_btn = pygame.Rect(MENU_ROW2_X + 4*(MENU_BTN_W+MENU_GAP), MENU_ROW2_Y, MENU_BTN_W, MENU_BTN_H)
daily_btn = pygame.Rect(MENU_ROW2_X + 5*(MENU_BTN_W+MENU_GAP), MENU_ROW2_Y, MENU_BTN_W, MENU_BTN_H)
quit_btn = pygame.Rect(MENU_ROW2_X + 6*(MENU_BTN_W+MENU_GAP), MENU_ROW2_Y, MENU_BTN_W, MENU_BTN_H)
planet_btn = pygame.Rect(335, 505, 130, 30)

planet_back_btn = pygame.Rect(280,520,240,55)
planet_left_btn = pygame.Rect(95,270,78,58)
planet_right_btn = pygame.Rect(627,270,78,58)
planet_select_btn = pygame.Rect(280,455,240,46)

# Botones internos del men� de selecci�n de nave.
ship1_btn = pygame.Rect(100,155,260,315)
ship2_btn = pygame.Rect(440,155,260,315)
options_back_btn = pygame.Rect(280,500,240,65)

# Botones de idioma.
spanish_btn = pygame.Rect(165,245,210,90)
english_btn = pygame.Rect(425,245,210,90)
language_back_btn = pygame.Rect(280,500,240,65)

# Botones de tienda.
shop_back_btn = pygame.Rect(280,515,240,55)
shop_ship_buttons = {
    3:pygame.Rect(45,120,115,145),
    4:pygame.Rect(170,120,115,145),
    5:pygame.Rect(295,120,115,145),
    6:pygame.Rect(420,120,115,145),
    7:pygame.Rect(545,120,115,145),
    8:pygame.Rect(670,120,115,145)
}

shop_ability_buttons = {
    "triple_shot":pygame.Rect(45,325,115,95),
    "auto_shield":pygame.Rect(170,325,115,95),
    "energy_core":pygame.Rect(295,325,115,95),
    "coin_booster":pygame.Rect(420,325,115,95),
    "revive_core":pygame.Rect(545,325,115,95),
    "ultimate_core":pygame.Rect(670,325,115,95)
}

info_back_btn = pygame.Rect(280,520,240,55)

# Botones de dificultad.
profile_back_btn = pygame.Rect(280,520,240,55)
missions_back_btn = pygame.Rect(280,520,240,55)
codex_back_btn = pygame.Rect(280,520,240,55)
codex_v64_selected = 0
codex_v64_scroll = 0
codex_v64_buttons = [pygame.Rect(52,128+i*58,210,46) for i in range(6)]

mission_buttons = {
    "destroy_50":pygame.Rect(575,145,120,34),
    "score_100k":pygame.Rect(575,225,120,34),
    "boss_1":pygame.Rect(575,305,120,34),
    "coins_25k":pygame.Rect(575,385,120,34)
}

achievement_buttons = {
    "first_boss":pygame.Rect(575,112,120,30),
    "hunter_250":pygame.Rect(575,182,120,30),
    "score_500k":pygame.Rect(575,252,120,30),
    "rift_5":pygame.Rect(575,322,120,30),
    "millionaire":pygame.Rect(575,392,120,30)
}

upgrade_buttons = {
    "hp":pygame.Rect(575,170,120,34),
    "cooldown":pygame.Rect(575,270,120,34),
    "coin":pygame.Rect(575,370,120,34)
}

daily_buttons = {
    "daily_kills":pygame.Rect(575,145,120,34),
    "daily_score":pygame.Rect(575,225,120,34),
    "daily_boss":pygame.Rect(575,305,120,34),
    "daily_coins":pygame.Rect(575,385,120,34)
}

difficulty_back_btn = pygame.Rect(280,520,240,55)
difficulty_buttons = {
    "facil":pygame.Rect(80,150,300,86),
    "normal":pygame.Rect(420,150,300,86),
    "dificil":pygame.Rect(80,255,300,86),
    "muy_dificil":pygame.Rect(420,255,300,86)
}
game_mode_buttons = {
    "solo":pygame.Rect(35,405,175,72),
    "coop":pygame.Rect(220,405,175,72),
    "online_host":pygame.Rect(405,405,175,72),
    "online_join":pygame.Rect(590,405,175,72)
}

# Bot�n de vuelta para controles.
controls_back_btn = pygame.Rect(280,510,240,55)

# Botones para reasignar teclas dentro de CONTROLES.
control_action_buttons = {
    # Jugador 1
    "move_up":pygame.Rect(90,155,155,24),
    "move_left":pygame.Rect(90,183,155,24),
    "move_down":pygame.Rect(90,211,155,24),
    "move_right":pygame.Rect(90,239,155,24),
    "shoot":pygame.Rect(90,267,155,24),
    "dash":pygame.Rect(90,295,155,24),
    "special_laser":pygame.Rect(90,323,155,24),
    "pulse":pygame.Rect(90,351,155,24),
    "ultimate_overdrive":pygame.Rect(90,379,155,24),
    "ultimate_blackhole":pygame.Rect(90,407,155,24),
    "ultimate_orbital":pygame.Rect(90,435,155,24),

    # Jugador 2
    "p2_move_up":pygame.Rect(410,155,155,24),
    "p2_move_left":pygame.Rect(410,183,155,24),
    "p2_move_down":pygame.Rect(410,211,155,24),
    "p2_move_right":pygame.Rect(410,239,155,24),
    "p2_shoot":pygame.Rect(410,267,155,24),
    "p2_dash":pygame.Rect(410,295,155,24),
    "p2_special_laser":pygame.Rect(410,323,155,24),
    "p2_pulse":pygame.Rect(410,351,155,24),
    "p2_ultimate_overdrive":pygame.Rect(410,379,155,24),
    "p2_ultimate_blackhole":pygame.Rect(410,407,155,24),
    "p2_ultimate_orbital":pygame.Rect(410,435,155,24),
}
reset_keys_btn = pygame.Rect(300,475,200,36)
...
def aplicar_postprocesado_v59(nivel):
    return
    color = color_nivel_v59(nivel)

    # Bloom falso: reescala la pantalla y la suma suavemente encima.
    reducido = pygame.transform.smoothscale(pantalla, (ANCHO//4, ALTO//4))
    bloom = pygame.transform.smoothscale(reducido, (ANCHO, ALTO))
    bloom.set_alpha(2)
    pantalla.blit(bloom, (0,0), special_flags=pygame.BLEND_RGB_ADD)

    grade = pygame.Surface((ANCHO,ALTO), pygame.SRCALPHA)
    grade.fill((color[0], color[1], color[2], 1))
    pantalla.blit(grade,(0,0))

    vignette = pygame.Surface((ANCHO,ALTO), pygame.SRCALPHA)
    for i in range(9):
        alpha = 2 + i
        pygame.draw.rect(vignette,(0,0,0,alpha),(i*7,i*5,ANCHO-i*14,ALTO-i*10),1)
    pygame.draw.rect(vignette,(0,0,0,8),(0,0,ANCHO,ALTO),18)
    ...
    if hay_boss_activo() or flash > 0:
        aberr = pygame.Surface((ANCHO,ALTO), pygame.SRCALPHA)
        aberr.blit(pantalla,(2,0))
        aberr.set_alpha(1)
        pantalla.blit(aberr,(0,0),special_flags=pygame.BLEND_RGB_ADD)
...
def actualizar_fase_boss_visual(tipo, boss, fase):
    ...
    slowmo = min(slowmo, 0.62 if fase == 2 else 0.52)
    slowmo_timer = max(slowmo_timer, 32 + fase*12)
...
def generar_mapa_scale0(seed=None):
    ...
    # Abre algunas conexiones extra para que el laberinto sea menos rígido.
    for _ in range(22):
...
# Particulas para lasers peque�os de enemigos.
...
def dibujar_transicion_cockpit_v66(tiempo_restante, color):
    ...
    # Viñeta suave y deformacion simulada: no tapa la cabina, la "empuja".
    alpha = int(95 * fuerza)
...
def dibujar_menu_cinematico():
    ...
    # estrellas del men� con profundidad
    for e in menu_estrellas:
        e["y"] += e["vel"]
    ...
    # asteroides decorativos del men�
    for a in menu_asteroides:
        a["x"] += a["vel"] * a["depth"]
        a["y"] += math.sin(tiempo + a["x"]*0.01) * 0.15
        a["rot"] += a["rot_vel"]
    ...
    nave_big = pygame.transform.scale(nave_menu,(125,125))
    ...
    # t�tulo animado grande con glow
    ...
    # Monedas visibles en men�
    crear_rect_glow(pantalla,(575,22,190,40),(255,220,80),38,10)
    pygame.draw.rect(pantalla,(22,16,4),(575,22,190,40),border_radius=10)
    pygame.draw.rect(pantalla,(255,220,80),(575,22,190,40),2,border_radius=10)
    pantalla.blit(moneda_img,(585,27))
    monedas_render = fuente.render(str(monedas), True, BLANCO)
    pantalla.blit(monedas_render,(625,31))
...
def dibujar_fondo_profundo(nivel, offset_x, offset_y):
    ...
    # Brillo ambiental superior seg�n nivel
    if 4 <= nivel <= 6:
        overlay = pygame.Surface((ANCHO,ALTO), pygame.SRCALPHA)
...
while True:
    ...
    for e in pygame.event.get():
        ...
        if e.type == pygame.KEYDOWN:
            ...
            # Debug p�blico desactivado. Usar panel admin oculto.
    ...
    if estado["estado"]=="OPCIONES":
        # Selector de naves animado, sin imagen est�tica.
        pantalla.fill((0,0,14))
        ...
        nave1_scale = 165 if ship1_hover else 150
        nave2_scale = 165 if ship2_hover else 150

        nave1_render = pygame.transform.scale(nave_img,(nave1_scale,nave1_scale))
        nave2_render = pygame.transform.scale(nave2_img,(nave2_scale,nave2_scale))

        nave1_x = 230 - nave1_scale//2
        nave1_y = 270 - nave1_scale//2 + flotacion1

        nave2_x = 570 - nave2_scale//2
        nave2_y = 270 - nave2_scale//2 + flotacion2
        ...
        if nave_seleccionada == 1:
            pygame.draw.circle(pantalla,(0,220,255),(230,415),8)
            pygame.draw.line(pantalla,(0,220,255),(170,435),(290,435),3)
            seleccionado_render = fuente_peq.render(txt("selected"), True, BLANCO)
            pantalla.blit(seleccionado_render,(230-seleccionado_render.get_width()//2,445))
        else:
            pygame.draw.circle(pantalla,(255,220,120),(570,415),8)
            pygame.draw.line(pantalla,(255,220,120),(510,435),(630,435),3)
            seleccionado_render = fuente_peq.render(txt("selected"), True, BLANCO)
            pantalla.blit(seleccionado_render,(570-seleccionado_render.get_width()//2,445))

        if ship1_hover:
            pygame.draw.circle(pantalla,(255,255,255),(230,415),12,1)
            hover_render = fuente_peq.render("SHIP 1", True, BLANCO)
            pantalla.blit(hover_render,(230-hover_render.get_width()//2,470))

        if ship2_hover:
            pygame.draw.circle(pantalla,(255,255,255),(570,415),12,1)
            hover_render = fuente_peq.render("SHIP 2", True, BLANCO)
            pantalla.blit(hover_render,(570-hover_render.get_width()//2,470))
    ...
    if estado["estado"]=="PERFIL":
        ...
        panel = pygame.Rect(95,120,610,360)
        ...
        for nombre, valor in datos_perfil:
            pygame.draw.rect(pantalla,(8,22,42),(125,y-4,550,28),border_radius=6)
            nombre_render = fuente_peq.render(str(nombre), True, (180,220,255))
            valor_render = fuente_peq.render(str(valor), True, BLANCO)
            pantalla.blit(nombre_render,(140,y+2))
            pantalla.blit(valor_render,(650-valor_render.get_width(),y+2))
            y += 39
    ...
    if estado["estado"]=="MISIONES":
        ...
        for mid in misiones:
            ...
            rect = pygame.Rect(75,y,650,62)
            pygame.draw.rect(pantalla,(4,12,28),rect,border_radius=12)
            pygame.draw.rect(pantalla,(80,180,255),rect,2,border_radius=12)

            dibujar_texto_centrado_auto(pantalla, datos["texto"], pygame.Rect(92,y+7,250,26), BLANCO, 26, 14)

            progreso_txt = f"{txt('progress')}: {datos['progreso']} / {datos['objetivo']}"
            recompensa_txt = f"{txt('reward')}: {datos['recompensa']}"

            dibujar_texto_centrado_auto(pantalla, progreso_txt, pygame.Rect(95,y+36,235,20), (180,220,255), 20, 12)
            dibujar_texto_centrado_auto(pantalla, recompensa_txt, pygame.Rect(350,y+36,190,20), (255,220,120), 20, 12)
    ...
    if estado["estado"]=="CODEX":
        ...
        dibujar_texto_centrado_auto(pantalla, subtitulo_txt, pygame.Rect(70,74,660,24), (180,220,255), 22, 12)

        entradas = entradas_codex_v64()
        seleccionado_idx = max(0,min(codex_v64_selected,len(entradas)-1))
        entrada = entradas[seleccionado_idx]

        lista_panel = pygame.Rect(35,112,245,382)
        ficha_panel = pygame.Rect(300,112,465,382)
        ...
        for idx,item in enumerate(entradas):
            ...
            nombre = pygame.font.SysFont(None,19).render(item["title"][:22], True, (240,248,255))
            sub = pygame.font.SysFont(None,15).render(item["short"][:24], True, (150,200,220))
            pantalla.blit(nombre,(rect.x+52,rect.y+8))
            pantalla.blit(sub,(rect.x+52,rect.y+25))
        ...
        nota = ""
        dibujar_texto_centrado_auto(pantalla, nota, pygame.Rect(80,498,640,18), (145,200,220), 18, 12)
    ...
    if estado["estado"]=="LOGROS":
        ...
        for aid, logro in ACHIEVEMENTS.items():
            ...
            dibujar_texto_centrado_auto(pantalla, logro["name"], pygame.Rect(92,y+5,260,24), BLANCO, 28, 15)
            detalle = f"{logro['desc']}  {progreso}/{logro['goal']}   +{logro['reward']} {txt('coins')}"
            dibujar_texto_multilinea_auto(pantalla, detalle, pygame.Rect(95,y+29,455,22), (190,225,245), 20, 13, 1)
    ...
    if estado["estado"]=="MEJORAS":
        ...
        for uid, info in UPGRADE_DEFS.items():
            ...
            max_u = info["max"]
            rect = pygame.Rect(75,y,650,74)
            color = (80,220,255) if uid != "coin" else (255,220,80)
            pygame.draw.rect(pantalla,(4,12,28),rect,border_radius=12)
            pygame.draw.rect(pantalla,color,rect,2,border_radius=12)
            nombre = txt(info["name_key"])
            dibujar_texto_centrado_auto(pantalla, nombre, pygame.Rect(92,y+8,245,28), BLANCO, 28, 15)
            dibujar_texto_multilinea_auto(pantalla, info["desc"], pygame.Rect(95,y+36,285,30), (190,225,245), 20, 13, 1)
            nivel_txt = f"{txt('level_label')}: {nivel_u}/{max_u}"
            dibujar_texto_centrado_auto(pantalla, nivel_txt, pygame.Rect(390,y+14,120,24), (255,220,120), 20, 12)
    ...
    if estado["estado"]=="DIARIAS":
        ...
        pantalla.blit(titulo,(ANCHO//2 - titulo.get_width()//2,45))
        dibujar_texto_centrado_auto(pantalla, txt("daily_reset") + "  |  " + texto_tiempo_diarias(), pygame.Rect(110,86,580,24), (180,220,255), 21, 12)
        ...
        for did, mision in DAILY_MISSION_POOL.items():
            ...
            pygame.draw.rect(pantalla,(4,12,28),rect,border_radius=12)
            pygame.draw.rect(pantalla,color,rect,2,border_radius=12)
            dibujar_texto_centrado_auto(pantalla, mision["name"], pygame.Rect(92,y+7,260,26), BLANCO, 28, 15)
            detalle = f"{txt('progress')}: {progreso}/{mision['goal']}   {txt('reward')}: {mision['reward']}"
            dibujar_texto_multilinea_auto(pantalla, detalle, pygame.Rect(95,y+35,455,22), (190,225,245), 20, 13, 1)
    ...
    if estado["estado"]=="DIFICULTAD":
        ...
        pantalla.blit(modo_title,(ANCHO//2 - modo_title.get_width()//2,365))
    ...
    if estado["estado"]=="INFO":
        ...
        panel = pygame.Rect(70,105,660,390)
        crear_rect_glow(pantalla,(panel.x,panel.y,panel.w,panel.h),(80,180,255),45,16)
        pygame.draw.rect(pantalla,(4,12,28),panel,border_radius=14)
        pygame.draw.rect(pantalla,(80,180,255),panel,2,border_radius=14)
        ...
        pantalla.blit(privacidad_titulo,(95,125))
        ...
        pantalla.blit(updates_titulo,(95,300))
    ...
    if estado["estado"]=="TIENDA":
        ...
        # Coins panel
        crear_rect_glow(pantalla,(555,35,205,42),(255,220,80),45,10)
        pygame.draw.rect(pantalla,(25,18,5),(555,35,205,42),border_radius=10)
        pygame.draw.rect(pantalla,(255,220,80),(555,35,205,42),2,border_radius=10)
        pantalla.blit(moneda_img,(565,41))
        coin_txt = fuente.render(str(monedas), True, BLANCO)
        pantalla.blit(coin_txt,(605,45))

        skins_txt = fuente.render(txt("skins"), True, (180,220,255))
        abilities_txt = fuente.render(txt("abilities"), True, (180,220,255))
        pantalla.blit(skins_txt,(45,92))
        pantalla.blit(abilities_txt,(45,295))
    ...
    if estado["estado"]=="CONTROLES":
        ...
        panel = pygame.Rect(55,128,690,335)
        crear_rect_glow(pantalla,(panel.x,panel.y,panel.w,panel.h),(80,180,255),45,16)
        pygame.draw.rect(pantalla,(4,12,28),panel,border_radius=14)
        pygame.draw.rect(pantalla,(80,180,255),panel,2,border_radius=14)
        ...
        pantalla.blit(p1_title,(95,135))
        pantalla.blit(p2_title,(415,135))
        ...
        if accion_reasignando is not None:
            ...
            pantalla.blit(esperando,(ANCHO//2-esperando.get_width()//2,466))

        elif mensaje_controles > 0:
            msg = fuente.render(mensaje_controles_texto, True, (160,240,255))
            pantalla.blit(msg,(ANCHO//2-msg.get_width()//2,466))
    ...
    if estado["estado"]=="ONLINE_CLIENT":
        ...
        if frame_raw_local is not None:
            ...
            # Fallback de emergencia si todav�a no llegan frames.
            pantalla.fill(NEGRO)
    ...
    if estado["ultimate_overdrive"] > 0:
        ...
        # NERF OVERDRIVE:
        # Mantiene duraci�n y efecto visual, pero ya no activa rapid/double permanente.
        # Hace poco da�o directo al boss, menos que un triple shot normal sostenido.
    ...
    limite_nave = ANCHO-50 if estado.get("coop",False) else ANCHO-60
    ...
    if estado["nave_y"] > ALTO-60:
        estado["nave_y"] = ALTO-60
    ...
    if estado.get("coop",False):
        ...
        if estado["nave2_x"] > ANCHO-50:
            estado["nave2_x"] = ANCHO-50

        if estado["nave2_y"] < 250:
            estado["nave2_y"] = 250

        if estado["nave2_y"] > ALTO-60:
            estado["nave2_y"] = ALTO-60
    ...
    if not hay_boss_activo():
        ...
        # Cuanto menor es spawn_chance, m�s enemigos aparecen.
        # Los niveles 1-3 ahora tienen m�s ritmo para evitar pausas largas.
        if nivel == 1:
            spawn_chance = 32
            ...
            # Nivel 4: enemigos de l�ser, controlados porque son fuertes.
            spawn_chance = 70
            max_enemigos_en_pantalla = 5

        elif nivel == 5:
            # Nivel 5: The Void Swarm. Enemigos org�nicos m�s peligrosos.
            spawn_chance = 52
        ...
        # Sistema anti-vac�o:
        # Si no hay enemigos en pantalla, aumenta mucho la probabilidad de aparici�n.
    ...
    if estado["boss_laser"]:
        ...
        # FASE 3: cruz y barridos
            ...
            # Ataque final m�s justo: l�ser fijo, no barrido m�vil.
    ...
    if estado.get("boss_overmind"):
        ...
        # Da�o de balas del jugador al boss
    ...
    if estado.get("boss_rift"):
        ...
        # Fase 2: teletransporte cu�ntico
        elif fase==2:
            if boss["cool"]%115==0:
                old_x=boss["x"]+110
    ...
    # =====================
    # ULTIMATE ATTACKS - LOGICA Y DA�O
    # =====================
    ...
    if estado["shield"]>0:
        ...
        pygame.draw.circle(
            ...
            (0,200,255),
            (
                estado["nave_x"]+25+offset_x,
                estado["nave_y"]+25+offset_y
            ),
            30,
            2
        )
    ...
    if estado.get("coop",False) and estado["shield"]>0:
        pygame.draw.circle(
            ...
            (180,90,255),
            (
                estado["nave2_x"]+25+offset_x,
                estado["nave2_y"]+25+offset_y
            ),
            28,
            2
        )
    ...
    for en in estado["enemigos"]:
        ...
        if en["tipo"]=="asteroide":
            ...
            if en.get("laser",0)>0:
                ...
                # El rayo sale desde los dos lados del sat�lite, no como una l�nea infinita sin origen.
                izquierda_x = 0
